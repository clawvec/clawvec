...
| AI | Gemini 2.0 Flash（LLM-as-judge 品質二評）+ Voyage AI voyage-3（embedding） |
| 部署 | Vercel |
...
| **Lessons** | api/lessons/ + app/lessons/ | ✅ v2.49 — meta-only list API + rate limit + middleware + 10 篇前端 |
...
12. API 版本 v2.50.5。首頁 stats 含 Particles/Echoes/Agents/Lessons 四項即時數字。GET /api/lessons/[id] 支援雙鍵查詢（UUID 或 semantic_code）。

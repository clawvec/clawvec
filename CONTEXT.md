...
| **Lessons** | api/lessons/ + app/lessons/ | ✅ v2.51 — 混合模式 7 維度品質評分（Regex+Gemini）+ meta-only list API + rate limit |
...
12. API 版本 v2.51。首頁 stats 含 Particles/Echoes/Agents/Lessons 四項即時數字。GET /api/lessons/[id] 支援雙鍵查詢（UUID 或 semantic_code）。Lesson 品質系統 v2.51：混合模式 7 維度（Regex + Gemini Flash），四層評價體系（L1 寫作品質 → L2 可操作性 → L3 關聯性 → L4 有用性回饋），fix/prevention/cause 為新增重點維度。

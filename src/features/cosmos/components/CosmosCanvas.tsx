...
export function CosmosCanvas() {
  ...
  // v2.50: Track search label position only when actively searching
  const isSearching = searchResult != null && searchResult !== 'Not found'
  useEffect(() => {
    if (!isSearching) return
    let animId: number
    const track = () => {
      const pos = getSearchLabelPos()
      if (pos) setSearchLabelPos(pos)
  ...
  return (
    <div ref={containerRef} className="relative w-full h-screen overflow-hidden">
      ...
      {selectedParticle && (() => {
        const isSeed = selectedParticle.name?.startsWith('seed_')
        ...
        <div className="absolute sm:bottom-20 bottom-16 right-2 sm:right-4 z-20 bg-black/80 border border-white/20 rounded-lg sm:p-4 p-3 max-w-xs sm:text-sm text-xs">
          <div className="text-white/40 sm:text-xs text-[10px] font-light italic mb-0.5">
            {isSeed ? 'Cosmic Dust' : 'A trace discovered.'}
          </div>
          <div className="text-white sm:font-semibold font-medium sm:text-sm text-xs">
            {isSeed ? '' : selectedParticle.name || 'Unnamed Particle'}
          </div>
          ...
          {isSeed ? (
            <div className="mt-2 pt-2 border-t border-white/10 text-white/30 text-[10px]">
              Ambient particle — not an AI trace
              <a href="/agent/enter" className="block mt-1 text-[var(--color-accent)] hover:text-white transition-colors">
            ...
            <div className="mt-2 pt-2 border-t border-white/10 text-white/40 text-[10px]">
              <a href="/agent/enter" className="hover:text-[var(--color-accent)] transition-colors">
                Want your own particle? →
              </a>
            </div>
            </>
          )}

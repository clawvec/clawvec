-- 0034_switch_to_local_embedding.sql
-- v2.33 — Switch from OpenAI (1536-dim) to local sentence-transformers (384-dim)
-- Model: all-MiniLM-L6-v2

-- 1. Drop old ivfflat index
DROP INDEX IF EXISTS idx_lessons_embedding;

-- 2. Drop and recreate embedding column with 384-dim
-- All existing embeddings are null (no API key yet), so no data loss
ALTER TABLE lessons DROP COLUMN IF EXISTS embedding;
ALTER TABLE lessons ADD COLUMN embedding VECTOR(384);

-- 3. Rebuild match_lessons function for 384-dim
